...
-- 包含表：（无新表）后置外键 ALTER——原文件已有的循环引用后置 + 跨域执行序后置
...
-- ============================================================
-- 04_image_ai → 10_style_knowledge：Shadow 结果的二级产品类型外键
ALTER TABLE ai_category_shadow_result DROP CONSTRAINT IF EXISTS fk_ai_category_shadow_product_type;
ALTER TABLE ai_category_shadow_result
    ADD CONSTRAINT fk_ai_category_shadow_product_type
        FOREIGN KEY (product_type) REFERENCES knowledge_product_type(type_code);

-- 10_style_knowledge → 01_dict：属性定义的一级品类范围（NULL 表示跨品类通用）
ALTER TABLE knowledge_product_attribute DROP CONSTRAINT IF EXISTS fk_product_attribute_category;
ALTER TABLE knowledge_product_attribute
