...
/** 行级报价删除权限判定（权限 + 数据范围）。 */
function canDeleteRskuRow(row: Rsku): boolean {
...
const pageTitle = computed(() => {
  ...
  return r?.productName || r?.rspuCode || rspuId.value
})
...
const pageSubtitle = computed(() => {
  const r = detail.value?.rspu
  if (!r) return ''
  const parts = [r.categoryPath, resolveStyleName(r.positioningLabel), `创建于 ${r.createdAt || '-'}`]
  return parts.filter(Boolean).join(' · ')
})

/** 全部 RSKU 中的最低出厂价。 */
const minFactoryPrice = computed(() => {
  const prices = rskuList.value
    .map(r => r.factoryPrice)
...
<template>
  <PageContainer :title="pageTitle" :subtitle="pageSubtitle">
    <template #actions>
      <n-space>
        ...
        <template v-if="detail">
          <n-button
            v-if="canUpdateProduct && canManageProduct"
            size="small"
            @click="openEditModal"
          >
            编辑元数据
    ...
    <n-space vertical :size="16">
      ...
      <n-alert v-if="isReadOnly" type="info" :show-icon="true">
      ...
      <n-spin :show="loading">
        <template v-if="detail">
          <DetailHero
            ...
            :min-factory-price="minFactoryPrice"
          />
          ...
          <n-card :bordered="true">
            <n-tabs v-model:value="activeTab" type="line" animated>
              ...
              <n-tab-pane name="variant-rsku" tab="变体与报价">
                <VariantRskuTab
                  :variant-list="variantList"
                  :variant-loading="variantLoading"
                  :rsku-list="rskuList"
                  :rsku-loading="rskuLoading"
                  :can-create-variant="canUpdateProduct && canManageProduct"
                  :can-create-rsku="canCreateRskuForProduct"
                  :can-delete-rsku-row="canDeleteRskuRow"
                  @create-variant="openVariantModal"
                  @create-rsku="openRskuModal"
                  @batch-create-rsku="openBatchRskuModal"
                  @delete-rsku="(row: Rsku) => handleDeleteRsku(row.rskuId)"
                  @open-rsku="(row: Rsku) => router.push(`/products/${rspuId}/rsku/${row.rskuId}`)"
                />
              </n-tab-pane>

              <n-tab-pane name="images" tab="图片">
                <ImageGalleryTab :images="detail.images" :variant-list="variantList" />
              </n-tab-pane>

              <n-tab-pane name="relations" tab="搭配关系">
                <RelationTab
                  :official-matches="detail.officialMatches || []"
                  :matched-by="detail.matchedBy || []"
                  :can-add-relation="canUpdateProduct && canManageProduct"
                  :can-delete-relation="canUpdateProduct && canManageProduct"
                  @add-relation="openRelationModal"
                  @delete-relation="handleDeleteRelation"
                  @open-product="(id: string) => router.push(`/products/${id}`)"
                />
              </n-tab-pane>

              <n-tab-pane v-if="isPlatformStaff" name="ai" tab="AI 识别">
                <AiInsightTab
                  :style-matches="detail.styleMatches || []"
                  :recognitions="detail.recognitions || []"
                />
              </n-tab-pane>
            </n-tabs>
          </n-card>
